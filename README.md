...
- **Phase 4 ✅ 打包 DMG**：`./build_mac.sh` 一键出 `Recorpaster.dmg`（拖进 Applications、双击即用、无需 Python/终端）。仅 macOS arm64；去 torch 轻量化。
- Phase 3 ⏳ AI 润色（v1.1，可插拔开关，默认关闭）。
...
Whisper 对中文默认**不输出标点**，整段会粘成一串。本项目两步解决（本地、不依赖 AI 润色）：

1. **`initial_prompt` 风格引导**（引擎层）：给两个引擎的 `transcribe` 传一段“本身带标点”的中文（默认
   `以下是普通话的句子，请加上标点。`）。Whisper 会**跟随这段的标点风格**输出标点（不是把它当指令）。
   实测同一段 18s 录音：不加 0 个标点 → 加了 6 个标点、可顺读。这段提示是 `engine.Config.initial_prompt`
   字段（向后兼容新增）+ `settings.py` 的 `initial_prompt`，可改文案调风格；置空即关闭。
2. **轻量本地标点规整**（app 层）：把 ASCII 标点在“两侧都是汉字”时转成全角（`,`→`，`、`.`→`。`），
   不动英文 / 数字 / 小数点（如 `3,000`、`v1.2` 不变）。

句间分隔：VAD 按停顿切句，每段各自带标点，拼接即可顺读；口语转书面（更强改写）留给 **Phase 3**。

## 打包成 DMG（Phase 4，傻瓜式安装）

仅 **macOS arm64（Apple Silicon）**——mlx 只在 Apple Silicon 可用；Windows EXE 后续单独用 CI 做。

**一键打包**（产物 `Recorpaster.dmg`）：

```bash
./build_mac.sh          # PyInstaller 打包 → ad-hoc 签名 → hdiutil 制作 DMG
```

**安装**：双击 `Recorpaster.dmg` → 把 **Recorpaster** 拖进 **Applications** → 从启动台/应用程序双击运行。

**首次打开**（ad-hoc 签名/未公证，会被 Gatekeeper 拦）二选一：
- **右键 →“打开”**，在弹窗里再点“打开”；或
- 终端执行 `xattr -dr com.apple.quarantine /Applications/Recorpaster.app`

**授权**（授给 **.app**，比授终端干净）：系统设置 → 隐私与安全性 → **麦克风 / 辅助功能 / 输入监控** 各勾选 Recorpaster，然后重开 app。
- 首次按热键说话时会弹麦克风授权（弹窗显示 app 名 + 用途，来自 Info.plist 的 `NSMicrophoneUsageDescription`）。
- ⚠️ ad-hoc 签名的 app **重新打包后签名会变**，TCC 可能视作“新 app”导致已授权失效、需重新授权。正式发布用 Developer ID 签名 + 公证可避免。

**首次运行下载模型**：1.6GB 的 Whisper 模型**不打进包**，首次运行从 HuggingFace 下载到 `~/.cache/huggingface`，
悬浮窗与菜单栏显示“下载模型中…（约 1.6GB，请保持联网）”；无网会清晰报错而非白屏卡死。之后走本地缓存秒开。

**轻量化**：包内**不含 torch**（VAD 改用纯 onnxruntime 跑 silero 模型，断句行为与 torch 版逐事件一致）；
仅 macOS arm64、mlx-only（不打 faster-whisper/ctranslate2）。当前 `.app` ≈ 530MB（大头是 mlx 的 `mlx.metallib` 120MB + `libmlx` 21MB）。

**排错日志**：打包后的 app 无控制台，运行日志写到 `~/Library/Logs/Recorpaster.log`。

**公证**（仅公开发布需要，本轮未做，`build_mac.sh` 里留了注释钩子）：Apple 开发者账号($99/年) + Developer ID 证书 +
hardened runtime + 麦克风 entitlements + `notarytool submit --wait` + `stapler staple`。

## 代码仓库

`git@github.com:ssgzy/Recorpaster.git`（main 分支）。`.gitignore` 已排除：`__pycache__/`、虚拟环境、
模型权重 / 缓存、`*.m4a` 等音频、`.DS_Store`、本地配置 `config.json`（含将来的 API key）、运行日志与预览图。
打包 EXE/DMG 等到功能稳定、外观锁定后做 **Phase 4**。
